...
void run(nb_sys_args_t *args)
{
	...
	muggle_socket_evloop_handle_set_cb_conn(&ev_handle,
											&tcp_upstream_on_connect);
	muggle_socket_evloop_handle_set_cb_close(&ev_handle,
											 &tcp_upstream_on_close);
	muggle_socket_evloop_handle_set_cb_release(&ev_handle,
											   &tcp_upstream_on_release);
	muggle_socket_evloop_handle_set_cb_msg(&ev_handle,
										   &tcp_upstream_on_message);
	...
	// listen
	muggle_socket_context_t *ctx = nb_tcp_listen(args);
...
int main(int argc, char *argv[])
{
	// init components
	nb_sys_args_t args;
	if (!nb_init_components(argc, argv, "tcp_upstream", &args)) {
		exit(EXIT_FAILURE);
	}
